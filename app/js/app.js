...
function modificaFragranza(codice) {
  ...
  const rigaForn = fo => `<div class="nome"><b>${esc(fo.sigla)}</b>${fo.nome !== fo.sigla ? `<span>${esc(fo.nome)}</span>` : ''}</div><input name="cf:${esc(fo.sigla)}" value="${esc(f.codiciFornitore?.[fo.sigla] || '')}" inputmode="numeric" placeholder="—" aria-label="Codice ${esc(fo.sigla)}"><input name="pl:${esc(fo.sigla)}" type="number" min="0" step="0.01" inputmode="decimal" value="${esc(f.costi?.[fo.sigla] ?? '')}" placeholder="—" aria-label="€/litro ${esc(fo.sigla)}">`;
  const campiForn = stato.fornitori.length ? `<div class="tab-forn"><div class="int">Fornitore</div><div class="int">Codice prodotto</div><div class="int">€/litro</div>${stato.fornitori.map(rigaForn).join('')}${costoGenerico !== '' ? `<div class="nome"><b>—</b><span>senza fornitore</span></div><div></div><input name="pl:_" type="number" min="0" step="0.01" inputmode="decimal" value="${esc(costoGenerico)}" placeholder="—" aria-label="€/litro senza fornitore">` : ''}</div>` : '';
  d.innerHTML = `<h2>${codice ? 'Modifica referenza ' + codice : 'Nuova referenza'}</h2><form id="form-ref">
    ...
    <fieldset class="forn"><legend>Fornitori: codice prodotto e costo €/litro</legend>${campiForn || '<p class="muted piccolo-testo" style="margin:0">Nessun fornitore ancora: aggiungilo nella scheda Referenze e soglie, sezione Fornitori.</p>'}
      ${!stato.fornitori.length ? `<label class="campo">€/litro<input name="pl:_" type="number" min="0" step="0.01" inputmode="decimal" value="${esc(costoGenerico)}" placeholder="—"></label>` : ''}
